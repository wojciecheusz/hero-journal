      ...
      {visible.map(fac => {
        ...
        return (
          <div key={fac.id} id={`entity-${fac.id}`} className={`card${fac.pinned?" pinned":""}`} style={{ padding:"1rem 1.1rem", borderLeftWidth:2, borderLeftColor: rc+"55" }}>
            ...
            {fac.notes && !isEditing && (
              <p style={{ fontFamily:"Crimson Text,Georgia,serif", fontSize:"0.95rem", color:"var(--hj-text)", lineHeight:1.65, marginTop:"0.5rem", whiteSpace:"pre-wrap", wordBreak:"break-word", opacity:0.88 }}>{fac.notes}</p>
            )}
