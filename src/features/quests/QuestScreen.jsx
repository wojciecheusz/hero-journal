...
export default function QuestScreen({ quests, setZadania, openEntity }) {
  ...
  const cycle = id => setZadania(q => q.map(x => x.id === id ? { ...x, status: STATUS_CYCLE[x.status] || x.status } : x));
  ...
  return (
    <>
      <div className="card">
        <div className="sect-label">{Q.newTitle}</div>
        <div className="col">
          <input className="g-input" placeholder={Q.namePh} value={name} onChange={e => setImie(e.target.value)} onKeyDown={e => e.key === "Enter" && addQuest()}/>
          <input className="g-input" placeholder={Q.descPh} value={desc} onChange={e => setDesc(e.target.value)}/>
          <input className="g-input" placeholder={Q.rewardPh} value={reward} onChange={e => setNagroda(e.target.value)}/>
          <div className="row" style={{ justifyContent:"flex-end" }}><button className="btn-ghost" onClick={addQuest}>{Q.addBtn}</button></div>
        </div>
      </div>

      {quests.length === 0 && <div className="card empty-state">{Q.empty}</div>}

      {["Aktywne","Ukończone","Nieudane"].map(status => {
        const filtered = quests.filter(q => q.status === status); if (!filtered.length) return null;
        const displayStatus = Q.statusLabel[status] ?? status;
                    ...
                    <div className="flex1">
                      <div className="row" style={{ marginBottom:"0.3rem", flexWrap:"wrap", gap:"0.4rem" }}>
                        <input className="iedit flex1" style={{ fontFamily:"Cinzel,serif", fontSize:"0.95rem", fontWeight:700 }}
                          value={quest.name} onChange={e => upd(quest.id, "name", e.target.value)} placeholder={Q.editNamePh}/>
                        <span className={`badge ${statusClass}`} onClick={() => cycle(quest.id)}>{displayStatus}</span>
                      </div>
                      <input className="iedit" style={{ fontSize:"0.92rem", fontStyle:"italic" }}
                        value={quest.description || ""} onChange={e => upd(quest.id, "description", e.target.value)} placeholder={Q.editDescPh}/>
                      {quest.reward && <div style={{ fontFamily:"Cinzel,serif", fontSize:"0.52rem", letterSpacing:"0.1em", color:"var(--quest-reward)", marginTop:"0.3rem" }}>{Q.reward} {quest.reward}</div>}
                      {kroks.length > 0 && <div style={{ fontFamily:"Cinzel,serif", fontSize:"0.52rem", letterSpacing:"0.08em", marginTop:"0.2rem", opacity:0.6 }}>{Q.progress(doneCount, kroks.length)}</div>}
                    </div>
                  ...
                  {open && (
                    <div style={{ marginTop:"0.7rem" }}>
                      {kroks.map(krok => (
                        <div key={krok.id} className="checklist-item">
                          ...
                          <input className={`iedit flex1 checklist-text${krok.done ? " done" : ""}`} style={{ fontSize:"0.92rem" }}
                            value={krok.text} onChange={e => updStep(quest.id, krok.id, "text", e.target.value)} placeholder={Q.stepPh}/>
                      ...
                      <div className="row mt05" style={{ justifyContent:"space-between", alignItems:"flex-end" }}>
                        <button className="btn-sm" onClick={() => addStep(quest.id)}>{Q.addStep}</button>
                        <div style={{ display:"flex", flexDirection:"column", gap:"0.1rem" }}>
                          <span style={{ fontFamily:"Cinzel,serif", fontSize:"0.48rem", letterSpacing:"0.1em", textTransform:"uppercase", opacity:0.6 }}>{Q.modifyReward}</span>
                          <input className="iedit" style={{ fontSize:"0.88rem", color:"var(--quest-reward)", minWidth:120 }}
                            value={quest.reward || ""} onChange={e => upd(quest.id, "reward", e.target.value)} placeholder={Q.rewardEditPh}/>
                        </div>
                      </div>
                    </div>
                  )}
