...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hj-root {
    ...
    background: #0f0d0b; color: #ddd5bb;
  ...
  .hj-logo {
    ...
    color: #e2b94e; letter-spacing: 0.12em;
  ...
  .sess-rendered:empty::before {
    content: attr(data-placeholder);
    color: #4a3e24;
    font-style: italic;
  }

  /* ── Entity links inside session text ── */
  .entity-link {
    cursor: pointer;
    border-radius: 0;
    padding: 0 2px;
  ...
  .entity-link-npc {
    ...
    border-bottom: 1px solid rgba(226,185,78,0.4);
  }
  .entity-link-npc:hover { background: rgba(226,185,78,0.2); border-bottom-color: #e2b94e; }
  .entity-link-location {
    color: #7aaccc;
    background: rgba(122,172,204,0.1);
    border-bottom: 1px solid rgba(122,172,204,0.4);
  }
  .entity-link-location:hover { background: rgba(122,172,204,0.2); border-bottom-color: #7aaccc; }
  ...
  .sess-legend {
    ...
    background: rgba(226,185,78,0.03);
...
/* ═══════════════════════════════════════════════
   ENTITY LINK PARSER
   Scans session notes for NPC names and location
   names, returns React nodes with clickable spans.
═══════════════════════════════════════════════ */
function parseEntityLinks(text, npcs, locations, onNavigate) {
  if (!text) return null;

  // Build sorted entity list (longer names first to avoid partial matches)
  const entities = [
    ...
    if (match) {
      return (
        <span
          ...
          title={`Go to ${match.type === 'npc' ? 'People' : 'Places'} → ${match.name}`}
        >
          {part}
        </span>
      );
...
function StatBox({ label, value, onChange }) {
  ...
  return (
    <div className={`stat-box${editing?" editing":""}`} onClick={()=>{if(!editing){setDraft(String(value));setEditing(true);}}}>
      ...
      {editing
        ? <input ref={ref} className="stat-input" value={draft} autoFocus onChange={e=>setDraft(e.target.value)} onBlur={commit} onKeyDown={e=>{if(e.key==="Enter")commit();if(e.key==="Escape")setEditing(false);}} onClick={e=>e.stopPropagation()}/>
...
function NPCTracker({ npcs, setNPCs }) {
  ...
  return (
    <>
          ...
          <div className="col">
            ...
            <div className="row" style={{gap:"0.5rem",flexWrap:"wrap"}}>
              {["unknown","ally","neutral","hostile"].map(r=>(
                <button key={r} className={`rel-badge rel-${r}`} style={{opacity:formState.relation===r?1:0.45}} onClick={()=>setForm(f=>({...f,relation:r}))}>{REL_LABELS[r]}</button>
              ))}
            </div>
      ...
      {visible.map(npc=>{
        const open=!!expanded[npc.id]; const rel=npc.relation||"unknown";
        return (
  ...
  return (
    <>
      ...
      <div className="filter-bar">
        ...
        {SKILL_CATS.map(c=>{ const count=skills.filter(s=>s.category===c).length; if(count===0)return null; return <button key={c} className={`filter-tag${activeCat===c?" active-filter":""}`} style={{borderColor:activeCat===c?catColor(c)+"88":"",color:activeCat===c?catColor(c):""}} onClick={()=>setActiveCat(activeCat===c?null:c)}>{c} ({count})</button>; })}
      </div>
      <FilterBar allTags={allTags} activeTag={activeTag} onSelect={setActiveTag}/>
      ...
      {visible.map(sk=>{
        const open=!!expanded[sk.id]; const cc=catColor(sk.category);
        return (
...
/* ═══════════════════════════════════════════════
   SESSION LOG — SMART (Sprint 3)
═══════════════════════════════════════════════ */
function SessionLog({ sessions, setSessions, npcs, locations, onNavigate }) {
  const [openIds,   setOpenIds]   = useState({});
  const [editingId, setEditingId] = useState(null); // which session is in edit mode
  ...
  // Count how many unique entities appear in all session notes
  const allNPCNames = npcs.map(n=>n.name).filter(Boolean);
  const allLocNames = locations.map(l=>l.name).filter(Boolean);

  const hasEntities = allNPCNames.length > 0 || allLocNames.length > 0;
  ...
  return (
    <>
      ...
      {/* Legend — only shown when there are entities to link */}
      {hasEntities && sessions.some(s=>s.notes) && (
        <div className="sess-legend">
          <span style={{fontFamily:"Cinzel,serif",fontSize:"0.5rem",letterSpacing:"0.1em",color:"#4a3a20",textTransform:"uppercase",marginRight:"0.2rem"}}>Links:</span>
          {allNPCNames.length>0&&(
            <div className="sess-legend-item">
              <div className="legend-dot" style={{background:"rgba(226,185,78,0.3)",border:"1px solid rgba(226,185,78,0.5)"}}/>
              <span style={{color:"#8a7040"}}>People</span>
            </div>
          )}
          {allLocNames.length>0&&(
            <div className="sess-legend-item">
              <div className="legend-dot" style={{background:"rgba(122,172,204,0.3)",border:"1px solid rgba(122,172,204,0.5)"}}/>
              <span style={{color:"#5a8aa0"}}>Places</span>
            </div>
          )}
          <span style={{fontFamily:"Crimson Text,serif",fontSize:"0.78rem",color:"#4a3a20",fontStyle:"italic",marginLeft:"0.3rem"}}>— tap to jump</span>
        </div>
      )}
      ...
      {sessions.map(sess=>{
        ...
        const parsed  = parseEntityLinks(sess.notes, npcs, locations, onNavigate);
          ...
          <div key={sess.id} className="sess-entry">
            ...
            {open&&(
              <div className="sess-body">
                {/* Toggle between rendered (linked) view and edit textarea */}
                {editing ? (
                  <>
                    <textarea
                      ...
                      rows={6}
                      autoFocus
                      placeholder="What transpired this session? Names of NPCs and locations you've added will become clickable links…"
                      value={sess.notes}
                  ...
                  <>
                    <div
                      className="sess-rendered"
                      data-placeholder="No notes yet — tap to write…"
                      onClick={()=>setEditingId(sess.id)}
                      title="Tap to edit"
                    >
  ...
  return (
    <>
      ...
      {["Active","Completed","Failed"].map(status=>{
        const filtered=quests.filter(q=>q.status===status); if(filtered.length===0)return null;
...
export default function HeroJournal() {
  ...
  // Navigate to a tab from within session log links
  const handleNavigate = useCallback(targetTab => setTab(targetTab), []);
