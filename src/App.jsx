...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card.pinned {
    border-color: #7a6030;
    box-shadow: 0 3px 0 #0a0806, 0 0 0 1px rgba(226,185,78,0.12), inset 0 1px 0 rgba(226,185,78,0.08);
  }
  .card.pinned::before {
    content: '';
    position: absolute;
    ...
    height: 2px;
    background: linear-gradient(90deg, transparent, #e2b94e, transparent);
    pointer-events: none;
  }
  ...
  .tag-default {
    background: rgba(226,185,78,0.08);
  ...
  .filter-tag.active-filter {
    background: rgba(226,185,78,0.1);
    border-color: #8a6830;
    color: #e2b94e;
  }
  ...
  .loc-type {
    ...
    border: 1px solid #352e1e;
    color: #7a6840;
    background: rgba(226,185,78,0.04);
    flex-shrink: 0;
  }
...
═══════════════════════════════════════════════ */
const DEFAULT_CHAR = {
  name: "Unnamed Hero", charClass: "Adventurer", level: 1,
...
function FilterBar({ allTags, activeTag, onSelect }) {
  ...
  return (
    <div className="filter-bar">
      ...
      {allTags.map(tag => (
        <button key={tag} className={`filter-tag${activeTag === tag ? " active-filter" : ""}`} onClick={() => onSelect(activeTag === tag ? null : tag)}>{tag}</button>
      ))}
    </div>
  );
}
...
function PinBtn({ pinned, onToggle }) {
  return (
    <button className={`pin-btn${pinned ? " pinned" : ""}`} onClick={onToggle} title={pinned ? "Unpin" : "Pin to top"}>
      {pinned ? "📌" : "📍"}
    </button>
  );
}
...
function StatBox({ label, value, onChange }) {
  ...
  return (
    <div className={`stat-box${editing ? " editing" : ""}`} onClick={() => { if (!editing) { setDraft(String(value)); setEditing(true); } }}>
      ...
      {editing
        ? <input ref={inputRef} className="stat-input" value={draft} autoFocus onChange={e => setDraft(e.target.value)} onBlur={commit} onKeyDown={e => { if (e.key==="Enter") commit(); if (e.key==="Escape") setEditing(false); }} onClick={e => e.stopPropagation()} />
...
const hpColor = pct => pct > 60 ? "linear-gradient(90deg,#6b1515,#9b2c2c,#cc3a3a)" : pct > 25 ? "linear-gradient(90deg,#7a3a10,#c06020,#d87030)" : "linear-gradient(90deg,#3a0a0a,#6b0f0f,#961a1a)";
    ...
    <>
      ...
      <div className="card">
        ...
        <div className="stat-grid">
          {STAT_KEYS.map(k => <StatBox key={k} label={k} value={char.stats[k]} onChange={v=>updSt(k,v)}/>)}
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="hp-bar-bg">
          <div className="hp-bar-fill" style={{width:`${hpPct}%`,background:hpColor(hpPct),boxShadow:`0 0 10px ${hpPct>25?"rgba(200,60,60,0.5)":"rgba(120,15,15,0.4)"}`}}/>
        </div>
...
function NPCTracker({ npcs, setNPCs }) {
  const [form,       setForm]       = useState({ name:"", role:"", relation:"unknown", notes:"" });
  ...
  const addNPC = () => {
    ...
    setNPCs(list => [...list, { id:Date.now(), name:n, role:form.role.trim(), relation:form.relation, notes:form.notes.trim(), tags:[], pinned:false }]);
    setForm({ name:"", role:"", relation:"unknown", notes:"" }); setShowForm(false);
  };

  const upd    = (id,f,v) => setNPCs(list => list.map(x=>x.id===id?{...x,[f]:v}:x));
  const del    = id => setNPCs(list => list.filter(x=>x.id!==id));
  const toggle = id => setExpanded(e => ({...e,[id]:!e[id]}));
  const cycleRel = id => setNPCs(list => list.map(x=>x.id===id?{...x,relation:REL_CYCLE[x.relation||"unknown"]}:x));
    ...
    <>
      ...
      {showForm && (
        <div className="add-form">
          ...
          <div className="col">
            <input className="g-input" placeholder="Name…" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addNPC()}/>
            <input className="g-input" placeholder="Role / occupation…" value={form.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))}/>
            <div className="row" style={{gap:"0.5rem",flexWrap:"wrap"}}>
              {["unknown","ally","neutral","hostile"].map(r=>(
                <button key={r} className={`rel-badge rel-${r}${form.relation===r?" ":" "}`} style={{opacity:form.relation===r?1:0.45}} onClick={()=>setForm(f=>({...f,relation:r}))}>{REL_LABELS[r]}</button>
              ))}
            </div>
            <textarea className="g-textarea" rows={3} placeholder="First impressions, known facts…" value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/>
            ...
            <div className="entity-header">
              <div className="flex1">
                <div className="row" style={{gap:"0.5rem",marginBottom:"0.25rem",flexWrap:"wrap"}}>
                  ...
                  <span className={`rel-badge rel-${rel}`} onClick={()=>cycleRel(npc.id)} title="Tap to change relation">{REL_LABELS[rel]}</span>
                </div>
            ...
            {open && (
              <div style={{marginTop:"0.8rem"}}>
                ...
                <div className="row mt05" style={{justifyContent:"flex-end"}}>
                  <button className="btn-ghost" onClick={()=>del(npc.id)}>Remove</button>
                </div>
              </div>
            )}
          </div>
...
function Locations({ locations, setLocations }) {
  ...
  const addLoc = () => {
    const n = form.name.trim(); if (!n) return;
    setLocations(list => [...list, { id:Date.now(), name:n, type:form.type, notes:form.notes.trim(), tags:[], pinned:false }]);
    setForm({ name:"", type:"Settlement", notes:"" }); setShowForm(false);
  };

  const upd    = (id,f,v) => setLocations(list => list.map(x=>x.id===id?{...x,[f]:v}:x));
  const del    = id => setLocations(list => list.filter(x=>x.id!==id));
  const toggle = id => setExpanded(e => ({...e,[id]:!e[id]}));
  ...
  return (
    <>
        ...
        <div className="add-form">
          ...
          <div className="col">
            ...
            <div className="row" style={{gap:"0.4rem",flexWrap:"wrap"}}>
              {LOC_TYPES.map(t=>(
                <button key={t} className="filter-tag" style={{opacity:form.type===t?1:0.45,borderColor:form.type===t?"#8a6830":"#352e1e",color:form.type===t?"#e2b94e":"#6a5830"}} onClick={()=>setForm(f=>({...f,type:t}))}>{t}</button>
              ))}
            </div>
      ...
      {locations.length===0 && <div className="card empty-state">No locations recorded.<br/><span style={{fontSize:"0.62rem"}}>Mark the first place your party has visited.</span></div>}
            ...
            {open && (
              <div style={{marginTop:"0.8rem"}}>
                <div className="row" style={{gap:"0.4rem",flexWrap:"wrap",marginBottom:"0.7rem"}}>
                  {LOC_TYPES.map(t=>(
                    <button key={t} className="filter-tag" style={{opacity:loc.type===t?1:0.4,borderColor:loc.type===t?"#8a6830":"#352e1e",color:loc.type===t?"#e2b94e":"#6a5830"}} onClick={()=>upd(loc.id,"type",t)}>{t}</button>
                  ))}
                </div>
                ...
                <div className="row mt05" style={{justifyContent:"flex-end"}}>
                  <button className="btn-ghost" onClick={()=>del(loc.id)}>Remove</button>
                </div>
              </div>
            )}
          </div>
...
function SkillPips({ value, onChange }) {
  return (
    <div className="skill-pips">
      {[1,2,3,4,5].map(i => (
        <div key={i} className={`pip${i<=value?" filled":""}`} onClick={()=>onChange(i===value?0:i)} title={`Level ${i}`}/>
      ))}
    </div>
  );
}
...
function SkillsTraits({ skills, setSkills }) {
  ...
  const addSkill = () => {
    const n = form.name.trim(); if (!n) return;
    setSkills(list => [...list, { id:Date.now(), name:n, category:form.category, description:form.description.trim(), level:form.level, tags:[], pinned:false }]);
    setForm({ name:"", category:"Skill", description:"", level:0 }); setShowForm(false);
  };

  const upd    = (id,f,v) => setSkills(list => list.map(x=>x.id===id?{...x,[f]:v}:x));
  const del    = id => setSkills(list => list.filter(x=>x.id!==id));
  const toggle = id => setExpanded(e => ({...e,[id]:!e[id]}));

  const visible = skills
    .filter(s => (!activeTag || (s.tags||[]).includes(activeTag)) && (!activeCat || s.category===activeCat))
    .sort((a,b)=>(b.pinned?1:0)-(a.pinned?1:0));
      ...
      {showForm && (
        <div className="add-form">
          ...
          <div className="col">
            ...
            <div className="row" style={{gap:"0.4rem",flexWrap:"wrap"}}>
              {SKILL_CATS.map(c=>(
                <button key={c} className="filter-tag" style={{opacity:form.category===c?1:0.45,borderColor:form.category===c?catColor(c)+"88":"#352e1e",color:form.category===c?catColor(c):"#6a5830"}} onClick={()=>setForm(f=>({...f,category:c}))}>{c}</button>
              ))}
            </div>
      ...
      {visible.map(sk => {
        ...
        return (
          <div key={sk.id} className={`card${sk.pinned?" pinned":""}`} style={{padding:"1rem 1.1rem", borderLeftColor: cc+"55", borderLeftWidth:2}}>
            ...
            {!open && sk.description && (
              <p style={{fontSize:"0.92rem",color:"#9a8a68",fontStyle:"italic",marginTop:"0.3rem",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{sk.description}</p>
            )}
            ...
            {open && (
              <div style={{marginTop:"0.8rem"}}>
                <div className="row" style={{gap:"0.4rem",flexWrap:"wrap",marginBottom:"0.5rem"}}>
                  {SKILL_CATS.map(c=>(
                    <button key={c} className="filter-tag" style={{opacity:sk.category===c?1:0.4,borderColor:sk.category===c?catColor(c)+"88":"#352e1e",color:sk.category===c?catColor(c):"#6a5830"}} onClick={()=>upd(sk.id,"category",c)}>{c}</button>
                  ))}
                </div>
                ...
                <div className="row mt05" style={{justifyContent:"flex-end"}}>
                  <button className="btn-ghost" onClick={()=>del(sk.id)}>Remove</button>
                </div>
              </div>
            )}
          </div>
...
function SessionLog({ sessions, setSessions }) {
  ...
  const addSession = () => {
    const entry = { id:Date.now(), number:sessions.length+1, date:today(), title:`Session ${sessions.length+1}`, notes:"" };
          ...
          <div key={sess.id} className="sess-entry">
            ...
            {open && (
              <div className="sess-body">
                <textarea className="g-textarea" rows={5} placeholder="What transpired this session? Record the deeds, discoveries, and disasters…" value={sess.notes} onChange={e=>upd(sess.id,"notes",e.target.value)}/>
                <div className="row mt05" style={{justifyContent:"flex-end"}}><button className="btn-ghost" onClick={()=>del(sess.id)}>Expunge Record</button></div>
              </div>
            )}
          </div>
        );
                ...
                <div className="row" style={{alignItems:"flex-start"}}>
                  <div className="flex1">
                    <div className="row" style={{marginBottom:"0.35rem",flexWrap:"wrap",gap:"0.4rem"}}>
                      ...
                      <span className={`badge ${status.toLowerCase()}`} onClick={()=>cycle(quest.id)} title="Tap to cycle status">{status}</span>
                    </div>
