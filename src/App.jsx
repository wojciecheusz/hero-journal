...
function buildCSS(t) {
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hj-root { --spell-accent: ${t.spellAccent}; --spell-border: ${t.spellBorder}; --spell-muted: ${t.spellMuted}; --spell-dim: ${t.spellDim}; --spell-text: ${t.spellText}; --spell-bg: ${t.spellBg}; --quest-reward: ${t.questNagroda}; }
...
const DEFAULT_CHAR = {
  ...
  przBonus:2, hp:{current:10,max:10,temp:0}, ac:10,
...
function EntityLink({ match, part, onNavigate, tabLabel }) {
  ...
  return (
    <span
      classImię={`entity-link entity-link-${match.type}`}
    ...
    >
      {part}
      {visible && (
        <span classImię="entity-tooltip" onClick={e=>e.stopPropagation()}>
          <span classImię="entity-tooltip-name">{match.name}</span>
          {match.tooltip.sub && (
            <span classImię="entity-tooltip-sub">{match.tooltip.sub}</span>
          )}
          {match.tooltip.body && (
            <span classImię="entity-tooltip-body">
...
function TagsEditor({tags,onChange}) {
  ...
  return <div classImię="tags-row">
    {tags.map(tag=><span key={tag} classImię="tag tag-default">{tag}<button classImię="tag-remove" onClick={()=>onChange(tags.filter(x=>x!==tag))}>✕</button></span>)}
    {adding?<input classImię="tag-input" autoFocus value={draft} placeholder="tag…" onChange={e=>setDraft(e.target.value)} onBlur={commit} onKeyDown={e=>{if(e.key==="Enter")commit();if(e.key==="Escape"){setAdding(false);setDraft("");}}}/>
           :<button classImię="tag-add-btn" onClick={()=>setAdding(true)}>+ tag</button>}
  </div>;
}

function FilterBar({allTags,activeTag,onSelect}) {
  if (!allTags.length) return null;
  return <div classImię="filter-bar">
    <span style={{fontFamily:"Cinzel,serif",fontSize:"0.52rem",letterSpacing:"0.12em",textTransform:"uppercase"}}>Filtr:</span>
    <button classImię={`filter-tag${!activeTag?" active-filter":""}`} onClick={()=>onSelect(null)}>Wszystkie</button>
    {allTags.map(tag=><button key={tag} classImię={`filter-tag${activeTag===tag?" active-filter":""}`} onClick={()=>onSelect(activeTag===tag?null:tag)}>{tag}</button>)}
  </div>;
}

function PrzypnijBtn({pinned,onToggle}) {
  return <button classImię={`pin-btn${pinned?" pinned":""}`} onClick={onToggle} title={pinned?"Odepnij":"Przypnij"}>{pinned?"📌":"📍"}</button>;
}

function Toggle({on,onToggle,label,color}) {
  const cls = on ? (color==="purple"?"on-purple":color==="blue"?"on-blue":"on") : "";
  return <div classImię="toggle-wrap" onClick={onToggle}>
    <div classImię={`toggle-track${cls?" "+cls:""}`}><div classImię="toggle-thumb"/></div>
    <span classImię="toggle-label">{label}</span>
  </div>;
}
...
function StatBox({label,value,onChange}) {
  ...
  return <div classImię={`stat-box${editing?" editing":""}`} onClick={()=>{if(!editing){setDraft(String(value));setEditing(true);}}}>
    <span classImię="stat-name">{label}</span>
    {editing?<input classImię="stat-input" value={draft} autoFocus onChange={e=>setDraft(e.target.value)} onBlur={commit} onKeyDown={e=>{if(e.key==="Enter")commit();if(e.key==="Escape")setEditing(false);}} onClick={e=>e.stopPropagation()}/>
            :<span classImię="stat-val">{value}</span>}
    <span classImię="stat-mod">{statMod(value)}</span>
  </div>;
}

function SkillPips({value,onChange}) {
  return <div classImię="skill-pips">{[1,2,3,4,5].map(i=><div key={i} classImię={`pip${i<=value?" filled":""}`} onClick={()=>onChange(i===value?0:i)}/>)}</div>;
}

/* ── Rest Modal ── */
function RestModal({ type, char, setChar, onClose }) {
  const pb = char.przBonus||2;
  ...
  return (
    <div classImię="modal-overlay" onClick={onClose}>
      <div classImię="modal-box" onClick={e=>e.stopPropagation()}>
        <div classImię="modal-title">{isShort?"☽ Krótki Odpoczynek":"☀ Długi Odpoczynek"}</div>
        {isShort ? <>
          {/* Krótki Odpoczynek — available dice info */}
          <p classImię="modal-text">
          ...
          <div style={{display:"flex",alignPrzedmioty:"center",gap:"0.6rem",marginBottom:"0.8rem",flexWrap:"wrap"}}>
            ...
            <select classImię="g-select" style={{width:"auto",fontSize:"0.9rem",padding:"0.3rem 0.5rem"}} value={hd.type}
          ...
          {/* Wydaj krokper */}
          <div classImię="modal-detail">
            <div style={{display:"flex",alignPrzedmioty:"center",gap:"0.5rem",marginBottom:"0.35rem"}}>
          ...
          <div classImię="row" style={{justifyContent:"flex-end",gap:"0.6rem",marginTop:"0.8rem"}}>
            <button classImię="btn-ghost" onClick={onClose}>Anuluj</button>
            <button classImię="btn-gold" onClick={doShortRest}>☽ Odpoczywaj</button>
          </div>
        </> : <>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"0.5rem",marginBottom:"1rem"}}>
            {[
              ...
              <div key={label} style={{display:"flex",alignPrzedmioty:"center",gap:"0.6rem",padding:"0.35rem 0",borderBottom:`1px solid rgba(128,128,128,0.15)`}}>
          ...
          <div classImię="row" style={{justifyContent:"flex-end",gap:"0.6rem"}}>
            <button classImię="btn-ghost" onClick={onClose}>Anuluj</button>
            <button classImię="btn-gold" onClick={doLongRest}>☀ Długi Odpoczynek</button>
          </div>
        </>}
      </div>
    </div>
...
function ResetModal({onConfirm,onAnuluj}) {
  return <div classImię="modal-overlay" onClick={onAnuluj}>
    <div classImię="modal-box" onClick={e=>e.stopPropagation()}>
      <div classImię="modal-title">"⚠ Pełny Reset"</div>
      <p classImię="modal-text">"Spowoduje to trwałe usunięcie wszystkich danych postaci. Operacji nie można cznąć."</p>
      <div classImię="row" style={{justifyContent:"flex-end",gap:"0.6rem"}}>
        <button classImię="btn-ghost" onClick={onAnuluj}>Anuluj</button>
        <button classImię="btn-danger" onClick={onConfirm}>Usuń Wszystko</button>
      </div>
    </div>
  </div>;
}
  ...
  return (
    <div style={{display:"grid",gap:"0.35rem",gridTemplateColumns:`repeat(${usedPozioms.length},1fr)`}}>
      {usedPozioms.map(lv=>{
        ...
        return (
          <div key={lv} classImię="spell-slot-box">
            <span classImię="spell-slot-label">{lv}</span>
            <div style={{display:"flex",alignPrzedmioty:"center",justifyContent:"center",gap:"2px"}}>
              <input classImię="spell-slot-input" type="number" min={0} value={sl.used||0}
              ...
              <input classImię="spell-slot-input" type="number" min={0} value={sl.max||0}
...
function PostaćSheet({ char, setChar, inventory, skills, spells }) {
  ...
  const [activeTab, setAktywnyTab] = useState("items"); // subtab in Aktywne i Wyposażone
  ...
  return <>
    ...
    {/* ══ CHARACTER card ══ */}
    <div classImię="card">
      <div classImię="sect-label">"Postać"</div>

      {/* Imię */}
      <div style={{marginBottom:"1rem"}}>
        <input classImię="iedit" style={{fontFamily:"Cinzel,serif",fontSize:"1.4rem",fontWeight:700,letterSpacing:"0.04em"}} value={char.name||""} onChange={e=>upd("name",e.target.value)} placeholder="Wpisz imię bohatera…"/>
      </div>

      {/* Classes */}
      <div style={{display:"flex",flexDirection:"column",gap:"0.3rem",marginBottom:"0.8rem"}}>
        {(char.classes||[]).map((cls,i)=>(
          <div key={i} classImię="class-row">
            <input classImię="iedit flex1" style={{fontFamily:"Cinzel,serif",fontSize:"0.95rem",color:i===0?"var(--accent)":"inherit",fontWeight:600}} value={cls.name} onChange={e=>setChar(c=>{const cl=[...c.classes];cl[i]={...cl[i],name:e.target.value};return{...c,classes:cl};})} placeholder={`Class ${i+1}…`}/>
            <span style={{fontFamily:"Cinzel,serif",fontSize:"0.56rem",letterSpacing:"0.1em",flexShrink:0}}>LVL</span>
            <input type="number" classImię="iedit" style={{width:38,textAlign:"center",fontFamily:"Cinzel,serif",fontSize:"0.95rem"}} value={cls.level} min={1} max={20} onChange={e=>setChar(c=>{const cl=[...c.classes];cl[i]={...cl[i],level:clamp(parseInt(e.target.value)||1,1,20)};return{...c,classes:cl};})}/>
            {i>0&&<button classImię="btn-ghost" style={{padding:"0.1rem 0.35rem",fontSize:"0.65rem"}} onClick={()=>setChar(c=>({...c,classes:c.classes.filter((_,j)=>j!==i)}))}>✕</button>}
          </div>
        ))}
        {(char.classes||[]).length<4&&<button classImię="btn-sm" style={{alignSelf:"flex-start",marginTop:"0.2rem"}} onClick={()=>setChar(c=>({...c,classes:[...(c.classes||[]),{name:"",level:1}]}))}>+ Multiclass</button>}
      </div>

      {/* Rodowód / PrzBonus / Charakter */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 90px 1fr",gap:"0.6rem",alignPrzedmioty:"end"}}>
        <div>
          ...
          <input classImię="iedit" style={{fontSize:"0.9rem"}} value={char.background||""} onChange={e=>upd("background",e.target.value)} placeholder="Rodowód…"/>
        </div>
        <div>
          ...
          <input type="number" classImię="iedit" style={{textAlign:"center",fontFamily:"Cinzel,serif",fontSize:"0.95rem"}} value={pb} onChange={e=>upd("przBonus",parseInt(e.target.value)||2)}/>
        </div>
        <div>
          ...
          <select classImię="g-select" value={char.alignment||"True Neutralny"} onChange={e=>upd("alignment",e.target.value)} style={{fontSize:"0.78rem",padding:"0.3rem 0.5rem"}}>{ALIGNMENTS.map(a=><option key={a} value={a}>{a}</option>)}</select>
        </div>
      </div>

      {/* ─ Attributes + ST inline — one grid, ST sits inside each stat box ─ */}
      <hr classImię="inner-divider" data-label="Cechy — dotknij aby edytować" style={{marginTop:"1.1rem"}}/>
      <div classImię="stat-grid-6" style={{marginTop:"0.8rem"}}>
        {SAVING_THROWS.map(st=>{
          ...
          return (
            <div key={st.key} style={{display:"flex",flexDirection:"column",gap:0}}>
              ...
              <div classImię="stat-box" style={{borderTop:"none",textAlign:"center",padding:"0.2rem 0.1rem",cursor:"default"}}>
      ...
      {/* ─ Żywotność ─ */}
      <hr classImię="inner-divider" data-label="Żywotność" style={{marginTop:"1.1rem"}}/>
      {/* HP row — minus | cur/max | plus | Temp HP inline */}
      <div style={{marginTop:"0.8rem",display:"flex",alignPrzedmioty:"center",gap:"0.4rem"}}>
        <button classImię="btn-pm minus" onClick={()=>setChar(c=>({...c,hp:{...c.hp,current:clamp(c.hp.current-1,0,c.hp.max)}}))}>−</button>
        <div classImię="hp-display">
        ...
        <button classImię="btn-pm plus" onClick={()=>setChar(c=>({...c,hp:{...c.hp,current:clamp(c.hp.current+1,0,c.hp.max)}}))}>+</button>
        {/* Temp HP — uses combat-box class for consistent styling */}
        <div classImię="combat-box" style={{marginLeft:"0.3rem",minWidth:56,padding:"0.25rem 0.4rem"}}>
          <span classImię="combat-box-label">Tmp HP</span>
          <input classImię="combat-box-input" type="number" value={char.hp.temp||0}
            onChange={e=>setChar(c=>({...c,hp:{...c.hp,temp:parseInt(e.target.value)||0}}))}/>
        </div>
      </div>
      {/* HP bar + pct */}
      <div classImię="hp-bar-bg" style={{marginTop:"0.5rem"}}><div classImię="hp-bar-fill" style={{width:`${hpPct}%`,background:hpBarColor(hpPct)}}/></div>
      <div classImię="hp-pct" style={{color:hpNumColor(hpPct)}}>{hpPct}% vitality remaining</div>
      {/* AC + Inicjatywa — full width below bar */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.5rem",marginTop:"0.5rem"}}>
        <div classImię="combat-box"><span classImię="combat-box-label">Klasa Pancerza</span><input classImię="combat-box-input" type="number" value={char.ac||10} onChange={e=>setChar(c=>({...c,ac:parseInt(e.target.value)||10}))}/></div>
        <div classImię="combat-box" title="DEX mod by default — edit to override"><span classImię="combat-box-label">Inicjatywa</span><input classImię="combat-box-input" type="number" value={char.initiativeBonus!==undefined?char.initiativeBonus:Math.floor((char.stats.DEX-10)/2)} onChange={e=>setChar(c=>({...c,initiativeBonus:parseInt(e.target.value)||0}))}/></div>
      </div>

      {/* Kości Wytrzymałości + Rest — grid: [HD tracker] [Short] [Long] */}
      <div style={{display:"grid",gridTemplateColumns:"auto 1fr 1fr",gap:"0.5rem",marginTop:"0.6rem",alignPrzedmioty:"stretch"}}>
        {/* Kości Wytrzymałości tracker — combat-box for consistent bg/border */}
        <div classImię="combat-box" style={{display:"flex",flexDirection:"column",alignPrzedmioty:"center",justifyContent:"center",padding:"0.3rem 0.6rem",gap:"0.15rem"}}>
          <span classImię="combat-box-label">Kości Wytrzymałości</span>
          <div style={{display:"flex",alignPrzedmioty:"center",gap:"0.25rem"}}>
            <select classImię="combat-box-input" style={{width:"auto",cursor:"pointer",fontSize:"0.78rem"}}
            ...
            <input type="number" min={0} value={(char.hitDice||{used:0}).used||0}
              ...
              classImię="combat-box-input" style={{width:26,borderBottom:"1px dashed currentColor"}}/>
            ...
            <input type="number" min={1} value={(char.hitDice||{max:1}).max||1}
              ...
              classImię="combat-box-input" style={{width:26,opacity:0.55}}/>
          </div>
        </div>
        {/* Krótki Odpoczynek */}
        <button classImię="btn-rest short" onClick={()=>setRestModal("short")}
          style={{display:"flex",flexDirection:"column",alignPrzedmioty:"center",justifyContent:"center",gap:"0.1rem",padding:"0.4rem 0.5rem"}}>
        ...
        {/* Długi Odpoczynek */}
        <button classImię="btn-rest long" onClick={()=>setRestModal("long")}
          style={{display:"flex",flexDirection:"column",alignPrzedmioty:"center",justifyContent:"center",gap:"0.1rem",padding:"0.4rem 0.5rem"}}>
          <span style={{fontSize:"1.1rem",lineHeight:1}}>☀</span>
      ...
      {/* ─ Rzuty Obronne ─ */}
      <hr classImię="inner-divider" data-label="Rzuty Obronne" style={{marginTop:"1.1rem"}}/>
      <div classImię="stat-grid-6" style={{marginTop:"0.8rem"}}>
        {SAVING_THROWS.map(st=>{
          const prz=!!(char.savingThrows||{})[st.key];
          const exp =!!(char.savingThrowExp||{})[st.key];
          ...
          const stateColor=exp?"#64c8e0":prz?"#e2b94e":"inherit";
          return (
            <div key={st.key} classImię={`stat-box${exp?" stat-box-exp":prz?" stat-box-prz":""}`}
              style={{cursor:"default",paddingTop:"0.5rem",paddingBottom:"0.5rem",position:"relative"}}>
              {przPip(prz,exp,()=>cycleSave(st.key))}
              <span classImię="stat-name">{st.key.toUpperCase()}</span>
              <span classImię="stat-val" style={{fontSize:"1.1rem",color:stateColor}}>{numMod(bonus)}</span>
              <span classImię="stat-mod" style={{fontSize:"0.52rem",color:stateColor,opacity:0.8}}>{exp?"ekspert":prz?"prz":"—"}</span>
            </div>
          );
        })}
      </div>
      ...
      {/* ─ Umiejętności ─ */}
      <hr classImię="inner-divider" data-label="Umiejętności" style={{marginTop:"1.1rem"}}/>
      <div classImię="save-grid" style={{marginTop:"0.8rem"}}>
        {GENERIC_SKILLS.map(sk=>{
          ...
          const bonus=exp?base+pb*2:prz?base+pb:base;
          return (
            <div key={sk.key} classImię="check-row">
              {przPip(prz,exp,()=>cycleSkill(sk.key))}
              <span classImię="check-attr">{sk.attr}</span>
              <span classImię="check-label" style={{color:exp?"#64c8e0":prz?"inherit":"inherit"}}>{sk.label}</span>
              <span classImię="check-bonus" style={{color:exp?"#64c8e0":prz?"#c9a84c":"inherit"}}>{numMod(bonus)}</span>
            </div>
          );
        })}
      </div>
      ...
      {(()=>{
        ...
        return <>
          <hr classImię="inner-divider" data-label="Miejsca na Czary" style={{marginTop:"1.1rem"}}/>
    ...
    {/* ══ Aktywne i Wyposażone with subtabs ══ */}
    {hasAktywny>0&&<div classImię="card">
      <div classImię="sect-label">Aktywne i Wyposażone</div>
      <div classImię="subtab-bar">
        {[["items","Przedmioty",equippedPrzedmioty.length],["skills","Umiejętności",activeUmiejętności.length],["spells","Czary",activeCzary.length]].map(([id,label,count])=>(
          count>0&&<button key={id} classImię={`subtab-btn${activeTab===id?" active":""}`} onClick={()=>setAktywnyTab(id)}>{label} ({count})</button>
        ))}
      </div>

      {activeTab==="items"&&equippedPrzedmioty.map(item=>(
        <div key={item.id} classImię="equipped-item">
          <span classImię="equipped-icon">{ITEM_ICONS[item.type]||"◈"}</span>
          <div classImię="flex1">
            <div classImię="row" style={{gap:"0.4rem",marginBottom:"0.15rem"}}><span classImię="equipped-name">{item.name}</span><span classImię="equipped-type-badge">{item.type}</span></div>
            {item.damage&&<div classImię="equipped-stat">Damage: {item.damage}{item.damageType?` ${item.damageType}`:""}</div>}
            {item.modifier!==undefined&&item.modifier!==""&&<div classImię="equipped-stat">Na Trafienie: {numMod(parseInt(item.modifier)||0)}</div>}
            {item.charges&&<div classImię="equipped-stat">Charges: {item.charges}</div>}
            {item.effect&&<div classImię="equipped-stat" style={{color:"#a87acc"}}>{item.effect}</div>}
          </div>
        </div>
      ))}

      {activeTab==="skills"&&activeUmiejętności.map(sk=>(
        <div key={sk.id} classImię="equipped-item">
          <span classImię="equipped-icon">✨</span>
          <div classImię="flex1">
            <div classImię="row" style={{gap:"0.4rem",marginBottom:"0.15rem"}}><span classImię="equipped-name">{sk.name}</span><span classImię="equipped-skill-badge">{sk.category}</span></div>
            {sk.description&&<div classImię="equipped-stat">{sk.description}</div>}
          </div>
        </div>
      ))}

      {activeTab==="spells"&&<>
        {/* Miejsca na Czary at top z Czary subtab */}
        {(()=>{
          const usedPozioms=[...new Set((spells||[]).map(s=>s.level).filter(l=>l!=="Cantrip"))];
          if (!usedPozioms.length) return null;
          return <div style={{marginBottom:"1rem"}}>
            <div style={{fontFamily:"Cinzel,serif",fontSize:"0.54rem",letterSpacing:"0.16em",textTransform:"uppercase",marginBottom:"0.4rem",color:"var(--spell-muted)"}}>Miejsca na Czary</div>
            <SpellSlotsWidget char={char} setChar={setChar} spells={spells}/>
          </div>;
        })()}
        {activeCzary.map(sp=>(
          <div key={sp.id} classImię="equipped-item">
            <span classImię="equipped-icon">🔮</span>
            <div classImię="flex1">
              <div classImię="row" style={{gap:"0.4rem",marginBottom:"0.15rem",flexWrap:"wrap"}}>
                <span classImię="equipped-name">{sp.name}</span>
                <span classImię="equipped-spell-badge">{sp.level}</span>
                {sp.school&&<span classImię="equipped-spell-badge">{sp.school}</span>}
              </div>
              {sp.castingTime&&<div classImię="equipped-stat">Cast: {sp.castingTime} · Zasięg: {sp.zakres||"—"}</div>}
              {sp.description&&<div classImię="equipped-stat">{sp.description.slice(0,100)}{sp.description.length>100?"…":""}</div>}
            </div>
          </div>
        ))}
      </>}
    ...
    {/* ══ Traits ══ */}
    <div classImię="card">
      <div classImię="sect-label">Cechy Charakteru</div>
      <div classImię="trait-grid">
        {[["personality","Cechy Osobowości","Jak twoja postać zachowuje się i mówi…"],
          ...
          <div key={key} classImię="trait-block">
            <span classImię="trait-label">{label}</span>
            <textarea classImię="trait-ta" rows={3} placeholder={ph} value={char.traits?.[key]||""} onChange={e=>setChar(c=>({...c,traits:{...(c.traits||{}),[key]:e.target.value}}))}/>
          </div>
        ))}
      </div>
    </div>
    ...
    <div classImię="card">
      <div classImię="sect-label">Notatki Osobiste</div>
      <textarea classImię="g-textarea" rows={4} placeholder="Przypomnienia z sesji, wskazówki MG, notatki drużyny…" value={char.personalNotes||""} onChange={e=>upd("personalNotes",e.target.value)}/>
    </div>

    <div classImię="card">
      <div classImię="sect-label">Historia Postaci</div>
      <textarea classImię="g-textarea" rows={6} placeholder="Skąd pochodzi twój bohater? Co go ukształtowało? Czego szuka?…" value={char.backstory||""} onChange={e=>upd("backstory",e.target.value)}/>
    </div>
  </>;
}
...
function Plecak({inventory,setInventory}) {
  ...
  return <>
    <div classImię="row" style={{justifyContent:"space-between"}}>
      ...
      <button classImię="btn-gold" onClick={()=>setShowForm(s=>!s)}>{showForm?"✕ Anuluj":"⊕ Dodaj Przedmiot"}</button>
    </div>
    {showForm&&<div classImię="add-form"><div classImię="col">
      <div classImię="row"><input classImię="g-input flex1" placeholder="Nazwa przedmiotu…" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addItem()}/><input classImię="g-input" style={{width:60}} placeholder="Ilość" value={form.qty} onChange={e=>setForm(f=>({...f,qty:e.target.value}))}/></div>
      <div classImię="row" style={{gap:"0.4rem",flexWrap:"wrap"}}>{ITEM_TYPES.map(t=><button key={t} classImię="filter-tag" style={{opacity:form.type===t?1:0.5,borderColor:form.type===t?"currentColor":""}} onClick={()=>setForm(f=>({...f,type:t}))}>{ITEM_ICONS[t]} {t}</button>)}</div>
      {needsExtras(form.type)&&<>
        {form.type==="Weapon"&&<div classImię="pack-item-row">
          <div classImię="pack-field"><span classImię="pack-field-label">Kości Obrażeń</span><input classImię="pack-field-input" placeholder="1d8" value={form.damage} onChange={e=>setForm(f=>({...f,damage:e.target.value}))}/></div>
          <div classImię="pack-field"><span classImię="pack-field-label">Typ Obrażeń</span><input classImię="pack-field-input" placeholder="Slashing" value={form.damageType} onChange={e=>setForm(f=>({...f,damageType:e.target.value}))}/></div>
          <div classImię="pack-field"><span classImię="pack-field-label">Na Trafienie</span><input classImię="pack-field-input" type="number" value={form.modifier} onChange={e=>setForm(f=>({...f,modifier:e.target.value}))}/></div>
        </div>}
        {["Spell Scroll","Wondrous Item","Consumable"].includes(form.type)&&<div classImię="pack-item-row">
          <div classImię="pack-field"><span classImię="pack-field-label">Charges</span><input classImię="pack-field-input" value={form.charges} onChange={e=>setForm(f=>({...f,charges:e.target.value}))}/></div>
          <div classImię="pack-field" style={{flex:2}}><span classImię="pack-field-label">Effect</span><input classImię="pack-field-input" value={form.effect} onChange={e=>setForm(f=>({...f,effect:e.target.value}))}/></div>
        </div>}
      </>}
      <input classImię="g-input" placeholder="Notatki…" value={form.note} onChange={e=>setForm(f=>({...f,note:e.target.value}))}/>
      <div classImię="row" style={{justifyContent:"flex-end"}}><button classImię="btn-gold" onClick={addItem}>⊕ Dodaj Przedmiot</button></div>
    </div></div>}
    <div classImię="filter-bar">
      <button classImię={`filter-tag${!filterType?" active-filter":""}`} onClick={()=>setFilterType(null)}>Wszystkie</button>
      {ITEM_TYPES.map(t=>{const c=inventory.filter(i=>i.type===t).length;if(!c)return null;return<button key={t} classImię={`filter-tag${filterType===t?" active-filter":""}`} onClick={()=>setFilterType(filterType===t?null:t)}>{ITEM_ICONS[t]} {t} ({c})</button>;})}
    </div>
    {inventory.length===0&&<div classImię="card empty-state">Twój plecak jest pusty…</div>}
    {visible.map(item=>{const open=!!expanded[item.id];return(
      <div key={item.id} classImię={`pack-item${item.equipped?" equipped-active":""}`}>
        <div classImię="pack-item-header">
          <span style={{fontSize:"1.1rem",flexShrink:0}}>{ITEM_ICONS[item.type]||"◈"}</span>
          <input classImię="iedit flex1" style={{fontFamily:"Cinzel,serif",fontSize:"0.9rem",fontWeight:700}} value={item.name} onChange={e=>upd(item.id,"name",e.target.value)}/>
          ...
          <button classImię="entity-toggle" onClick={()=>toggle(item.id)}>{open?"▲":"▼"}</button>
        </div>
        {open&&<div classImię="pack-item-body">
          <div classImię="pack-item-row">
            <div classImię="pack-field"><span classImię="pack-field-label">Ilość</span><input classImię="pack-field-input" value={item.qty||"1"} onChange={e=>upd(item.id,"qty",e.target.value)}/></div>
            {item.type==="Weapon"&&<>
              <div classImię="pack-field"><span classImię="pack-field-label">Damage</span><input classImię="pack-field-input" value={item.damage||""} onChange={e=>upd(item.id,"damage",e.target.value)}/></div>
              <div classImię="pack-field"><span classImię="pack-field-label">Type</span><input classImię="pack-field-input" value={item.damageType||""} onChange={e=>upd(item.id,"damageType",e.target.value)}/></div>
              <div classImię="pack-field"><span classImię="pack-field-label">Na Trafienie</span><input classImię="pack-field-input" type="number" value={item.modifier||""} onChange={e=>upd(item.id,"modifier",e.target.value)}/></div>
            </>}
            {["Spell Scroll","Wondrous Item","Consumable"].includes(item.type)&&<>
              <div classImię="pack-field"><span classImię="pack-field-label">Charges</span><input classImię="pack-field-input" value={item.charges||""} onChange={e=>upd(item.id,"charges",e.target.value)}/></div>
              <div classImię="pack-field" style={{flex:2}}><span classImię="pack-field-label">Effect</span><input classImię="pack-field-input" value={item.effect||""} onChange={e=>upd(item.id,"effect",e.target.value)}/></div>
            </>}
          </div>
          <div classImię="pack-field"><span classImię="pack-field-label">Notes</span><input classImię="pack-field-input" value={item.note||""} onChange={e=>upd(item.id,"note",e.target.value)}/></div>
          <div classImię="row" style={{justifyContent:"flex-end",marginTop:"0.3rem"}}><button classImię="btn-ghost" onClick={()=>del(item.id)}>Usuń</button></div>
        </div>}
      </div>
    );})}
  </>;
...
function UmiejętnościTab({skills,setUmiejętności}) {
  ...
  return <>
    <div classImię="row" style={{justifyContent:"space-between"}}>
      ...
      <button classImię="btn-gold" onClick={()=>setShowForm(s=>!s)}>{showForm?"✕ Anuluj":"⊕ Dodaj Wpis"}</button>
    </div>
    {showForm&&<div classImię="add-form"><div classImię="col">
      <input classImię="g-input" placeholder="Nazwa…" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addSkill()}/>
      <div classImię="row" style={{gap:"0.4rem",flexWrap:"wrap"}}>{SKILL_CATS.map(c=><button key={c} classImię="filter-tag" style={{opacity:form.category===c?1:0.45,borderColor:form.category===c?catColor(c)+"88":"",color:form.category===c?catColor(c):""}} onClick={()=>setForm(f=>({...f,category:c}))}>{c}</button>)}</div>
      <div classImię="row" style={{gap:"0.6rem"}}><span style={{fontFamily:"Cinzel,serif",fontSize:"0.58rem",textTransform:"uppercase",letterSpacing:"0.12em"}}>Mistrzostwo</span><SkillPips value={form.level} onChange={v=>setForm(f=>({...f,level:v}))}/></div>
      <textarea classImię="g-textarea" rows={3} placeholder="Opis, efekt, źródło, wymagania wstępne…" value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))}/>
      <div classImię="row" style={{justifyContent:"flex-end"}}><button classImię="btn-gold" onClick={addSkill}>⊕ Add</button></div>
    </div></div>}
    <div classImię="filter-bar">
      <button classImię={`filter-tag${!activeCat?" active-filter":""}`} onClick={()=>setAktywnyCat(null)}>Wszystkie</button>
      {SKILL_CATS.map(c=>{const count=skills.filter(s=>s.category===c).length;if(!count)return null;return<button key={c} classImię={`filter-tag${activeCat===c?" active-filter":""}`} style={{borderColor:activeCat===c?catColor(c)+"88":"",color:activeCat===c?catColor(c):""}} onClick={()=>setAktywnyCat(activeCat===c?null:c)}>{c} ({count})</button>;})}
    </div>
    <FilterBar allTags={allTags} activeTag={activeTag} onSelect={setAktywnyTag}/>
    {skills.length===0&&<div classImię="card empty-state">Brak zapisanych umiejętności, cech ani talentów.</div>}
    {visible.map(sk=>{const open=!!expanded[sk.id];const cc=catColor(sk.category);return(
      <div key={sk.id} classImię={`card${sk.pinned?" pinned":""}${sk.inUse?" inuse-active":""}`} style={{padding:"1rem 1.1rem",borderLeftColor:cc+"55",borderLeftWidth:2}}>
        <div classImię="entity-header">
          <div classImię="flex1">
            <div classImię="row" style={{gap:"0.5rem",marginBottom:"0.25rem",flexWrap:"wrap"}}>
              <input classImię="iedit flex1" style={{fontFamily:"Cinzel,serif",fontSize:"0.98rem",fontWeight:700}} value={sk.name} onChange={e=>upd(sk.id,"name",e.target.value)} placeholder="Nazwa…"/>
          ...
          <button classImię="entity-toggle" onClick={()=>toggle(sk.id)}>{open?"▲":"▼"}</button>
        </div>
        ...
        {open&&<div style={{marginTop:"0.8rem"}}>
          <div classImię="row" style={{gap:"0.4rem",flexWrap:"wrap",marginBottom:"0.5rem"}}>{SKILL_CATS.map(c=><button key={c} classImię="filter-tag" style={{opacity:sk.category===c?1:0.4,borderColor:sk.category===c?catColor(c)+"88":"",color:sk.category===c?catColor(c):""}} onClick={()=>upd(sk.id,"category",c)}>{c}</button>)}</div>
          <div classImię="row" style={{gap:"0.6rem",marginBottom:"0.7rem"}}><span style={{fontFamily:"Cinzel,serif",fontSize:"0.58rem",textTransform:"uppercase",letterSpacing:"0.12em"}}>Mistrzostwo</span><SkillPips value={sk.level} onChange={v=>upd(sk.id,"level",v)}/></div>
          <textarea classImię="g-textarea" rows={4} placeholder="Opis, efekt, źródło, wymagania…" value={sk.description||""} onChange={e=>upd(sk.id,"description",e.target.value)}/>
          <div classImię="row mt05" style={{justifyContent:"flex-end"}}><button classImię="btn-ghost" onClick={()=>del(sk.id)}>Usuń</button></div>
        </div>}
      </div>
    );})}
  </>;
...
function CzaryTab({spells,setCzary,char,setChar}) {
  ...
  return <>
    <div classImię="row" style={{justifyContent:"space-between"}}>
      ...
      <div classImię="row" style={{gap:"0.5rem"}}>
        <button classImię="btn-sm" style={{borderColor:"var(--spell-border)",color:"var(--spell-accent)"}} onClick={()=>setShowSlots(s=>!s)}>{showSlots?"✕ Slots":"⚙ Slots"}</button>
        <button classImię="btn-gold" onClick={()=>setShowForm(s=>!s)}>{showForm?"✕ Anuluj":"⊕ Dodaj Czar"}</button>
      </div>
    </div>

    {showSlots&&<div classImię="card" style={{borderColor:"var(--spell-border)"}}>
      <div classImię="sect-label" style={{color:"var(--spell-accent)"}}>Miejsca na Czary & Casting</div>
      <div style={{display:"flex",alignPrzedmioty:"center",gap:"0.6rem",marginBottom:"0.7rem",flexWrap:"wrap"}}>
        ...
        <select classImię="g-select" style={{width:"auto",fontSize:"0.82rem",padding:"0.25rem 0.5rem",borderColor:"#1a3a6a"}} value={char.spellcastingAbility||"INT"} onChange={e=>setChar(c=>({...c,spellcastingAbility:e.target.value}))}>{STAT_KEYS.map(s=><option key={s} value={s}>{s}</option>)}</select>
        <span style={{fontFamily:"Cinzel,serif",fontSize:"0.72rem",color:"var(--spell-accent)"}}>DC {8+pb+spMod} · Atk {numMod(pb+spMod)}</span>
      </div>
      <SpellSlotsWidget char={char} setChar={setChar} spells={spells}/>
    </div>}

    {showForm&&<div classImię="add-form" style={{borderColor:"var(--spell-border)"}}><div classImię="col">
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.5rem"}}>
        <input classImię="g-input" placeholder="Nazwa czaru…" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addSpell()}/>
        <select classImię="g-select" value={form.school} onChange={e=>setForm(f=>({...f,school:e.target.value}))}>{SPELL_SCHOOLS.map(s=><option key={s} value={s}>{s}</option>)}</select>
      </div>
      <div classImię="row" style={{gap:"0.4rem",flexWrap:"wrap"}}>{SPELL_LEVELS.map(lv=><button key={lv} classImię="filter-tag" style={{opacity:form.level===lv?1:0.45,borderColor:form.level===lv?"#1a5a9a":"",color:form.level===lv?"#64a0e6":""}} onClick={()=>setForm(f=>({...f,level:lv}))}>{lv}</button>)}</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"0.5rem"}}>
        <div classImię="pack-field"><span classImię="pack-field-label">Czas Rzucania</span><input classImię="pack-field-input" placeholder="1 action" value={form.castingTime} onChange={e=>setForm(f=>({...f,castingTime:e.target.value}))}/></div>
        <div classImię="pack-field"><span classImię="pack-field-label">Zasięg</span><input classImię="pack-field-input" placeholder="60 ft" value={form.zakres} onChange={e=>setForm(f=>({...f,zakres:e.target.value}))}/></div>
        <div classImię="pack-field"><span classImię="pack-field-label">Czas Trwania</span><input classImię="pack-field-input" placeholder="Instantaneous" value={form.duration} onChange={e=>setForm(f=>({...f,duration:e.target.value}))}/></div>
      </div>
      <input classImię="g-input" placeholder="Komponenty (W, G, M…)" value={form.components} onChange={e=>setForm(f=>({...f,components:e.target.value}))}/>
      <textarea classImię="g-textarea" rows={3} placeholder="Opis czaru i efekty…" value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))}/>
      <div classImię="row" style={{justifyContent:"flex-end"}}><button classImię="btn-gold" onClick={addSpell}>⊕ Dodaj Czar</button></div>
    </div></div>}

    <div classImię="filter-bar">
      <button classImię={`filter-tag${!activePoziom?" active-filter":""}`} onClick={()=>setAktywnyPoziom(null)}>Wszystkie</button>
      {SPELL_LEVELS.map(lv=>{const count=spells.filter(s=>s.level===lv).length;if(!count)return null;return<button key={lv} classImię={`filter-tag${activePoziom===lv?" active-filter":""}`} style={{borderColor:activePoziom===lv?"#1a5a9a":"",color:activePoziom===lv?"#64a0e6":""}} onClick={()=>setAktywnyPoziom(activePoziom===lv?null:lv)}>{lv} ({count})</button>;})}
    </div>

    {spells.length===0&&<div classImię="card empty-state">Brak zapisanych czarów.</div>}

    {visible.map(sp=>{const open=!!expanded[sp.id];return(
      <div key={sp.id} classImię={`card${sp.pinned?" pinned":""}${sp.inUse?" spell-active":""}`} style={{padding:"1rem 1.1rem",borderLeftColor:"#1a4a8a",borderLeftWidth:2}}>
        <div classImię="entity-header">
          <div classImię="flex1">
            <div classImię="row" style={{gap:"0.5rem",marginBottom:"0.3rem",flexWrap:"wrap"}}>
              <input classImię="iedit flex1" style={{fontFamily:"Cinzel,serif",fontSize:"0.98rem",color:"var(--spell-text)",fontWeight:700}} value={sp.name} onChange={e=>upd(sp.id,"name",e.target.value)} placeholder="Nazwa czaru…"/>
              <span classImię="spell-level-badge">{sp.level}</span>
              {sp.school&&<span classImię="spell-school-badge">{sp.school}</span>}
            </div>
          ...
          <button classImię="entity-toggle" onClick={()=>toggle(sp.id)}>{open?"▲":"▼"}</button>
        </div>
        ...
        {open&&<div style={{marginTop:"0.8rem"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.5rem",marginBottom:"0.7rem"}}>
            <div><span style={{fontFamily:"Cinzel,serif",fontSize:"0.5rem",letterSpacing:"0.12em",color:"var(--spell-muted)",textTransform:"uppercase",display:"block",marginBottom:"0.2rem"}}>Poziom</span><select classImię="g-select" style={{fontSize:"0.82rem",padding:"0.25rem 0.5rem",borderColor:"var(--spell-border)"}} value={sp.level} onChange={e=>upd(sp.id,"level",e.target.value)}>{SPELL_LEVELS.map(lv=><option key={lv} value={lv}>{lv}</option>)}</select></div>
            <div><span style={{fontFamily:"Cinzel,serif",fontSize:"0.5rem",letterSpacing:"0.12em",color:"var(--spell-muted)",textTransform:"uppercase",display:"block",marginBottom:"0.2rem"}}>School</span><select classImię="g-select" style={{fontSize:"0.82rem",padding:"0.25rem 0.5rem",borderColor:"var(--spell-border)"}} value={sp.school} onChange={e=>upd(sp.id,"school",e.target.value)}>{SPELL_SCHOOLS.map(s=><option key={s} value={s}>{s}</option>)}</select></div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"0.5rem",marginBottom:"0.7rem"}}>
            <div classImię="pack-field"><span classImię="pack-field-label">Czas Rzucania</span><input classImię="pack-field-input" value={sp.castingTime||""} onChange={e=>upd(sp.id,"castingTime",e.target.value)}/></div>
            <div classImię="pack-field"><span classImię="pack-field-label">Zasięg</span><input classImię="pack-field-input" value={sp.zakres||""} onChange={e=>upd(sp.id,"zakres",e.target.value)}/></div>
            <div classImię="pack-field"><span classImię="pack-field-label">Czas Trwania</span><input classImię="pack-field-input" value={sp.duration||""} onChange={e=>upd(sp.id,"duration",e.target.value)}/></div>
          </div>
          <div classImię="pack-field" style={{marginBottom:"0.6rem"}}><span classImię="pack-field-label">Components</span><input classImię="pack-field-input" value={sp.components||""} placeholder="V, S, M (material)" onChange={e=>upd(sp.id,"components",e.target.value)}/></div>
          <textarea classImię="g-textarea" rows={4} placeholder="Opis czaru…" value={sp.description||""} onChange={e=>upd(sp.id,"description",e.target.value)}/>
          <div style={{marginTop:"0.5rem"}}>
            ...
            <textarea classImię="g-textarea" rows={2} value={sp.notes||""} placeholder="Gdy rzucony wyższym miejscem na czar…" onChange={e=>upd(sp.id,"notes",e.target.value)}/>
          </div>
          <div classImię="row mt05" style={{justifyContent:"flex-end"}}><button classImię="btn-ghost" onClick={()=>del(sp.id)}>Usuń</button></div>
        </div>}
      </div>
    );})}
  </>;
...
function NPCTracker({npcs,setNPCs}) {
  ...
  return <>
    <div classImię="row" style={{justifyContent:"space-between"}}>
      ...
      <button classImię="btn-gold" onClick={()=>setShowForm(s=>!s)}>{showForm?"✕ Anuluj":"⊕ Dodaj Postać"}</button>
    </div>
    {showForm&&<div classImię="add-form"><div classImię="col">
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.5rem"}}>
        <input classImię="g-input" placeholder="Nazwa…" value={formState.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addNPC()}/>
        <input classImię="g-input" placeholder="Rola / zajęcie…" value={formState.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))}/>
        <input classImię="g-input" placeholder="Przynależność…" value={formState.affiliation} onChange={e=>setForm(f=>({...f,affiliation:e.target.value}))}/>
        <input classImię="g-input" placeholder="Poznano w…" value={formState.metAt} onChange={e=>setForm(f=>({...f,metAt:e.target.value}))}/>
      </div>
      <input classImię="g-input" placeholder="Powiązania…" value={formState.connections} onChange={e=>setForm(f=>({...f,connections:e.target.value}))}/>
      <div classImię="row" style={{gap:"0.5rem",flexWrap:"wrap"}}>{["unknown","ally","neutral","hostile"].map(r=><button key={r} classImię={`rel-badge rel-${r}`} style={{opacity:formState.relation===r?1:0.45}} onClick={()=>setForm(f=>({...f,relation:r}))}>{REL_LABELS[r]}</button>)}</div>
      <textarea classImię="g-textarea" rows={3} placeholder="Notatki…" value={formState.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/>
      <div classImię="row" style={{justifyContent:"flex-end"}}><button classImię="btn-gold" onClick={addNPC}>⊕ Add</button></div>
    </div></div>}
    <FilterBar allTags={allTags} activeTag={activeTag} onSelect={setAktywnyTag}/>
    {npcs.length===0&&<div classImię="card empty-state">Brak zapisanych postaci.</div>}
    {visible.map(npc=>{const open=!!expanded[npc.id];const rel=npc.relation||"unknown";return(
      <div key={npc.id} classImię={`card${npc.pinned?" pinned":""}`} style={{padding:"1rem 1.1rem"}}>
        <div classImię="entity-header">
          <div classImię="flex1">
            <div classImię="row" style={{gap:"0.5rem",marginBottom:"0.25rem",flexWrap:"wrap"}}>
              <input classImię="iedit flex1" style={{fontFamily:"Cinzel,serif",fontSize:"1rem",fontWeight:700}} value={npc.name} onChange={e=>upd(npc.id,"name",e.target.value)} placeholder="Nazwa…"/>
              <span classImię={`rel-badge rel-${rel}`} onClick={()=>cycleRel(npc.id)}>{REL_LABELS[rel]}</span>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.25rem 0.6rem"}}>
              <input classImię="iedit" style={{fontSize:"0.88rem",fontStyle:"italic"}} value={npc.role||""} onChange={e=>upd(npc.id,"role",e.target.value)} placeholder="Rola…"/>
              <input classImię="iedit" style={{fontSize:"0.88rem"}} value={npc.affiliation||""} onChange={e=>upd(npc.id,"affiliation",e.target.value)} placeholder="Przynależność…"/>
              <input classImię="iedit" style={{fontSize:"0.85rem"}} value={npc.metAt||""} onChange={e=>upd(npc.id,"metAt",e.target.value)} placeholder="Poznano w…"/>
              <input classImię="iedit" style={{fontSize:"0.85rem"}} value={npc.connections||""} onChange={e=>upd(npc.id,"connections",e.target.value)} placeholder="Powiązania…"/>
            </div>
          </div>
          <PrzypnijBtn pinned={npc.pinned} onToggle={()=>upd(npc.id,"pinned",!npc.pinned)}/>
          <button classImię="entity-toggle" onClick={()=>toggle(npc.id)}>{open?"▲":"▼"}</button>
        </div>
        <TagsEditor tags={npc.tags||[]} onChange={v=>upd(npc.id,"tags",v)}/>
        {open&&<div style={{marginTop:"0.8rem"}}>
          <textarea classImię="g-textarea" rows={4} placeholder="Znane fakty, osobowość, sekrety…" value={npc.notes||""} onChange={e=>upd(npc.id,"notes",e.target.value)}/>
          <div classImię="row mt05" style={{justifyContent:"flex-end"}}><button classImię="btn-ghost" onClick={()=>del(npc.id)}>Usuń</button></div>
        </div>}
      </div>
    );})}
  </>;
...
function Locations({locations,setLocations}) {
  ...
  return <>
    <div classImię="row" style={{justifyContent:"space-between"}}>
      ...
      <button classImię="btn-gold" onClick={()=>setShowForm(s=>!s)}>{showForm?"✕ Anuluj":"⊕ Dodaj Lokację"}</button>
    </div>
    {showForm&&<div classImię="add-form"><div classImię="col">
      <input classImię="g-input" placeholder="Nazwa lokacji…" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addLoc()}/>
      <div classImię="row" style={{gap:"0.4rem",flexWrap:"wrap"}}>{LOC_TYPES.map(t=><button key={t} classImię="filter-tag" style={{opacity:form.type===t?1:0.45,borderColor:form.type===t?"currentColor":""}} onClick={()=>setForm(f=>({...f,type:t}))}>{t}</button>)}</div>
      <textarea classImię="g-textarea" rows={3} placeholder="Opis, atmosfera, istotne elementy…" value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/>
      <div classImię="row" style={{justifyContent:"flex-end"}}><button classImię="btn-gold" onClick={addLoc}>⊕ Add</button></div>
    </div></div>}
    <FilterBar allTags={allTags} activeTag={activeTag} onSelect={setAktywnyTag}/>
    {locations.length===0&&<div classImię="card empty-state">Brak zapisanych lokacji.</div>}
    {visible.map(loc=>{const open=!!expanded[loc.id];return(
      <div key={loc.id} classImię={`card${loc.pinned?" pinned":""}`} style={{padding:"1rem 1.1rem"}}>
        <div classImię="entity-header">
          <div classImię="flex1">
            <div classImię="row" style={{gap:"0.5rem",marginBottom:"0.25rem"}}>
              <input classImię="iedit flex1" style={{fontFamily:"Cinzel,serif",fontSize:"1rem",fontWeight:700}} value={loc.name} onChange={e=>upd(loc.id,"name",e.target.value)} placeholder="Nazwa lokacji…"/>
              <span classImię="loc-type">{loc.type}</span>
            </div>
          </div>
          <PrzypnijBtn pinned={loc.pinned} onToggle={()=>upd(loc.id,"pinned",!loc.pinned)}/>
          <button classImię="entity-toggle" onClick={()=>toggle(loc.id)}>{open?"▲":"▼"}</button>
        </div>
        <TagsEditor tags={loc.tags||[]} onChange={v=>upd(loc.id,"tags",v)}/>
        {open&&<div style={{marginTop:"0.8rem"}}>
          <div classImię="row" style={{gap:"0.4rem",flexWrap:"wrap",marginBottom:"0.7rem"}}>{LOC_TYPES.map(t=><button key={t} classImię="filter-tag" style={{opacity:loc.type===t?1:0.4,borderColor:loc.type===t?"currentColor":""}} onClick={()=>upd(loc.id,"type",t)}>{t}</button>)}</div>
          <textarea classImię="g-textarea" rows={4} placeholder="Geografia, atmosfera, istotne cechy, zagrożenia…" value={loc.notes||""} onChange={e=>upd(loc.id,"notes",e.target.value)}/>
          <div classImię="row mt05" style={{justifyContent:"flex-end"}}><button classImię="btn-ghost" onClick={()=>del(loc.id)}>Usuń</button></div>
        </div>}
      </div>
    );})}
  </>;
...
function SesjaDziennik({sessions,setSesjas,npcs,locations,quests,inventory,skills,onNavigate}) {
  ...
  return <>
    ...
    {sessions.map(sess=>{
      ...
      return <div key={sess.id} classImię="sess-entry">
        <div classImię={`sess-header${open?" open":""}`} onClick={()=>toggle(sess.id)}>
          <span classImię="sess-num">#{String(sess.number).padStart(2,"0")}</span>
          <input classImię="iedit flex1" style={{fontFamily:"Cinzel,serif",fontSize:"0.92rem"}} value={sess.title} onChange={e=>{e.stopPropagation();upd(sess.id,"title",e.target.value);}} onClick={e=>e.stopPropagation()}/>
        ...
        {open&&<div classImię="sess-body">
          {editing?<>
            <textarea classImię="g-textarea" rows={6} autoFocus placeholder="Pisz notatki z sesji… imiona postaci i miejsc staną się klikalnymi linkami." value={sess.notes} onChange={e=>upd(sess.id,"notes",e.target.value)}/>
            <div classImię="row mt05" style={{justifyContent:"space-between"}}>
              <button classImię="btn-ghost" onClick={()=>del(sess.id)}>Usuń Zapis</button>
              <button classImię="btn-gold" onClick={()=>setEditingId(null)}>✓ Gotowe</button>
            </div>
          </>:<>
            <div classImię="sess-rendered" data-placeholder="Brak notatek — dotknij aby pisać…" onClick={()=>setEditingId(sess.id)}>{sess.notes?parsed:null}</div>
            <div classImię="row mt05" style={{justifyContent:"space-between"}}>
              <button classImię="btn-ghost" onClick={()=>del(sess.id)}>Usuń Zapis</button>
              <button classImię="btn-ghost" style={{opacity:0.7}} onClick={()=>setEditingId(sess.id)}>✎ Edytuj</button>
            </div>
          </>}
        </div>}
      </div>;
...
function QuestTracker({quests,setZadania}) {
  ...
  return <>
    <div classImię="card">
      <div classImię="sect-label">Nowe Zlecenie</div>
      <div classImię="col">
        <input classImię="g-input" placeholder="Nazwa zlecenia…" value={name} onChange={e=>setImię(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addQuest()}/>
        <input classImię="g-input" placeholder="Krótki opis…" value={desc} onChange={e=>setDesc(e.target.value)}/>
        <input classImię="g-input" placeholder="Nagroda (złoto, przedmioty, PD…)" value={reward} onChange={e=>setNagroda(e.target.value)}/>
        <div classImię="row" style={{justifyContent:"flex-end"}}><button classImię="btn-gold" onClick={addQuest}>⊕ Dodaj Zadanie</button></div>
      </div>
    </div>
    {quests.length===0&&<div classImię="card empty-state">Brak wydanych zleceń.</div>}
    {["Aktywny","Ukończone","Nieudane"].map(status=>{
      ...
      return <div key={status} style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>
        <div classImię="sect-label" style={{color:lc}}>{status} <span style={{opacity:0.5}}>({filtered.length})</span></div>
        {filtered.map(quest=>{const open=!!expanded[quest.id];const kroks=quest.kroks||[];const doneCount=kroks.filter(s=>s.done).length;
          return <div key={quest.id} classImię={`quest-entry ${status.toLowerCase()}`}>
            <div classImię="row" style={{alignPrzedmioty:"flex-start"}}>
              <div classImię="flex1">
                <div classImię="row" style={{marginBottom:"0.3rem",flexWrap:"wrap",gap:"0.4rem"}}>
                  <input classImię="iedit flex1" style={{fontFamily:"Cinzel,serif",fontSize:"0.95rem",fontWeight:700}} value={quest.name} onChange={e=>upd(quest.id,"name",e.target.value)} placeholder="Nazwa zlecenia…"/>
                  <span classImię={`badge ${status.toLowerCase()}`} onClick={()=>cycle(quest.id)}>{status}</span>
                </div>
                <input classImię="iedit" style={{fontSize:"0.92rem",fontStyle:"italic"}} value={quest.description||""} onChange={e=>upd(quest.id,"description",e.target.value)} placeholder="Opis…"/>
              ...
              <button classImię="entity-toggle" onClick={()=>toggle(quest.id)} style={{marginTop:"0.1rem"}}>{open?"▲":"▼"}</button>
            ...
            {open&&<div style={{marginTop:"0.7rem"}}>
              {kroks.map(krok=><div key={krok.id} classImię="checklist-item">
                <div classImię={`check-box${krok.done?" checked":""}`} onClick={()=>updStep(quest.id,krok.id,"done",!krok.done)}/>
                <input classImię={`iedit flex1 checklist-text${krok.done?" done":""}`} style={{fontSize:"0.92rem"}} value={krok.text} onChange={e=>updStep(quest.id,krok.id,"text",e.target.value)} placeholder="Opis kroku…"/>
              ...
              <div classImię="row mt05" style={{justifyContent:"space-between",alignPrzedmioty:"flex-end"}}>
                <button classImię="btn-sm" onClick={()=>addStep(quest.id)}>+ Krok</button>
                <div style={{display:"flex",flexDirection:"column",gap:"0.1rem"}}>
                  ...
                  <input classImię="iedit" style={{fontSize:"0.88rem",color:"var(--quest-reward)",minWidth:120}} value={quest.reward||""} onChange={e=>upd(quest.id,"reward",e.target.value)} placeholder="Złoto, przedmioty, PD…"/>
                </div>
              </div>
            </div>}
          </div>;
...
function PrzileScreen({ prziles, activeId, onSelect, onCreate, onDelete, theme }) {
  ...
  return (
    <div classImię="przile-screen">
      ...
      <div classImię="przile-logo">⚔ Bohater Journal</div>
      <div classImię="przile-tagline">Wybierz bohatera aby kontynuować przygodę</div>

      <div classImię="przile-list">
        {prziles.map(p => (
          <div key={p.id} classImię={`przile-card${p.id===activeId?" active-przile":""}`}
            onClick={() => onSelect(p.id)}>
            <span classImię="przile-card-icon">
              {DND_CLASSES.find(c=>c.name===p.class)?.icon || "⚔️"}
            </span>
            <div style={{flex:1,minWidth:0}}>
              <div classImię="przile-card-name">{p.name||"Bezimienny Bohater"}</div>
              <div classImię="przile-card-sub">
            ...
            {prziles.length > 1 && (
              <button classImię="przile-card-del" onClick={e=>{e.stopPropagation();onDelete(p.id);}}>✕</button>
            )}
          </div>
        ))}
      </div>

      <button classImię="btn-new-przile" onClick={onCreate}>
...
function PostaćWizard({ onFinish, onAnuluj, theme }) {
  ...
  return (
    <div classImię="wizard-screen">
      <div classImię="wizard-box">
        {/* Step dots */}
        <div classImię="wizard-krok-dots">
          {STEPS.map((s,i)=><div key={s} classImię={`wizard-dot${i<=krok?" active":""}`}/>)}
        </div>

        <div classImię="wizard-title">New Bohater</div>
        <div classImię="wizard-sub">{STEPS[krok]} — krok {krok+1} z {STEPS.length}</div>

        {/* Step 0: Imię */}
        {krok===0&&<>
          <div classImię="wizard-krok-label">Jak ma na imię twój bohater?</div>
        ...
        {/* Step 1: Class */}
        {krok===1&&<>
          <div classImię="wizard-krok-label">Wybierz klasę</div>
          <div classImię="wizard-class-grid">
            {DND_CLASSES.map(c=>(
              <button key={c.name} classImię={`wizard-class-btn${cls?.name===c.name?" selected":""}`}
                onClick={()=>setCls(c)}>
                <span classImię="wizard-class-icon">{c.icon}</span>
                <span classImię="wizard-class-name">{c.name}</span>
              </button>
            ))}
          </div>
          <div style={{display:"flex",alignPrzedmioty:"center",gap:"0.6rem",marginTop:"0.4rem"}}>
        ...
        {/* Step 2: Cechy */}
        {krok===2&&<>
          <div classImię="wizard-krok-label">Wybierz rozłożenie cech</div>
          ...
          <div classImię="wizard-stat-grid">
            {STAT_KEYS.map(k=>(
              <div key={k} classImię="wizard-stat-box">
                <span classImię="wizard-stat-label">{k}</span>
                {useWłasne
                  ...
                  : <span classImię="wizard-stat-val">{stats[k]}</span>}
        ...
        {/* Step 3: Szczegóły */}
        {krok===3&&<>
          <div classImię="wizard-krok-label">Rodowód</div>
          ...
          <div classImię="wizard-krok-label">Charakter</div>
...
export default function BohaterJournal() {
  ...
  return <>
    ...
    <div classImię="hj-root">

      <header classImię="hj-header">
        <div style={{maxWidth:780,margin:"0 auto",display:"flex",alignPrzedmioty:"center",justifyContent:"space-between",gap:"0.75rem"}}>
          <div style={{display:"flex",alignPrzedmioty:"center",gap:"0.75rem",minWidth:0,cursor:"pointer"}} onClick={()=>setScreen("prziles")} title="Zmień postać">
            <div classImię="hj-logo"><span style={{fontSize:"0.9rem",opacity:0.75}}>⚔</span>HJ</div>
            <span classImię="hj-char-name">{charImię||"Bohater"}</span>
          ...
          <div style={{display:"flex",alignPrzedmioty:"center",gap:"0.4rem",flexShrink:0}}>
            <button
            ...
            >{theme==="dark"?"☀ Pergamin":"🌙 Ciemny"}</button>
            <button classImię="btn-danger" style={{fontSize:"0.55rem",padding:"0.2rem 0.55rem",letterSpacing:"0.1em"}} onClick={()=>setShowReset(true)} title="Resetuj aktualną postać">↺ Reset</button>
          </div>
        </div>
      </header>

      <main classImię="hj-content">
      ...
      <nav classImię="hj-bottom-nav">
        {TABS.map(tb=><button key={tb.id} classImię={`hj-nav-btn${tab===tb.id?" active":""}`} onClick={()=>setTab(tb.id)}>
          <span classImię="hj-nav-icon">{tb.icon}</span>
          <span classImię="hj-nav-label">{tb.label}</span>
        </button>)}
      </nav>
    </div>
  </>;
