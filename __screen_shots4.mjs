...
const page = await browser.newPage({ viewport: { width: 1900, height: 1300 }, deviceScaleFactor: 1.5 });
...
const cols = await page.locator('.screen-col').all();
console.log('cols found:', cols.length);
for (let i=0;i<cols.length;i++){
  await cols[i].screenshot({ path: `__col_${i}.png` });
}
await browser.close();
